...
class MusicCommands(commands.Cog):
    ...
    async def moodplay(self, ctx, mood: str):
        """Play a song based on the specified mood"""
        mood = mood.lower()
        if mood not in self.mood_playlists:
        ...
        # Select a random song from the mood playlist
        selected_song = random.choice(self.mood_playlists[mood])

        # Create loading message
        loading_msg = await ctx.send(
            f"🎭 **Selected Mood:** `{mood}`\n"
            f"🎵 **Preparing to play:** `{selected_song}`\n"
            "⏳ Loading..."
        )
        ...
        try:
            ...
            # Get song results
            results = await self.get_song_results(selected_song)
            if not results:
                await loading_msg.edit(content=f"❌ Could not find song: {selected_song}")
                return

            song_info = results[0]  # Get first result

            # Join voice channel if not already joined
            if not ctx.author.voice:
                await loading_msg.edit(content="❌ You need to be in a voice channel first!")
                return
            ...
            # Create Now Playing embed
            playing_embed = discord.Embed(
                title=f"🎭 Now Playing ({mood.title()} Mood)",
            ...
            await loading_msg.edit(content=f"❌ An error occurred while playing the song: {str(e)}")
